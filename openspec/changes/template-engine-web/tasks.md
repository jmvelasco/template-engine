...
- [ ] 2.1 **REASON**: List and organize all domain test cases from the `template-parsing` spec. Write them as a TODO list in the test file. Validate with Tech Lead before starting
- [ ] 2.2 **TDD cycle**: `ParseResult` value object — parse a template with no placeholders returns unchanged text and empty notifications
