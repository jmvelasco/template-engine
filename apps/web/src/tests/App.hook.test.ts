...
import { useApp } from "../infrastructure/ui/App/App.hook";
...
describe("The useApp hook (App.hook.ts)", () => {
  ...
  test("initializes states with default values", () => {
    const { result } = renderHook(() => useApp(dummyUseCase));
  ...
  test("parses template successfully calling the usecase", async () => {
    ...
    const { result } = renderHook(() => useApp(useCase));
